...
async function clearTable(tableName) {
  ...
  const getResult = await makeSupabaseRequest('GET', `/${tableName}?select=id`);
  
  if (!getResult.success) {
    console.log(`   ❌ Failed to get records from ${tableName}`);
    return false;
  }

  let records = [];
  try {
    records = JSON.parse(getResult.data);
  } catch (e) {
    console.log(`   ❌ Failed to parse response from ${tableName}`);
    return false;
  }
  ...
  for (const record of records) {
    const deleteResult = await makeSupabaseRequest('DELETE', `/${tableName}?id=eq.${record.id}`);
    if (deleteResult.success) {
      deletedCount++;
    }
  }

  console.log(`   ✅ Deleted ${deletedCount}/${records.length} records from ${tableName}`);
...
async function clearSupabaseData() {
  ...
  results.push(await clearTable('interview_feedback'));

  // 2. Clear interviews (depends on users and candidates)
  results.push(await clearTable('interviews'));

  // 3. Clear candidates (depends on users)
  results.push(await clearTable('candidates'));

  // 4. Clear users (base table)
  results.push(await clearTable('users'));
