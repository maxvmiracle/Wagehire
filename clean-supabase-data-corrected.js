...
async function clearSupabaseData() {
  ...
  results.push(await makeSupabaseRequest('DELETE', '/interview_feedback'));

  // 2. Clear interviews (depends on users)
  console.log('📋 Step 2: Clearing interviews table...');
  results.push(await makeSupabaseRequest('DELETE', '/interviews'));

  // 3. Clear users (base table)
  console.log('📋 Step 3: Clearing users table...');
  results.push(await makeSupabaseRequest('DELETE', '/users'));
